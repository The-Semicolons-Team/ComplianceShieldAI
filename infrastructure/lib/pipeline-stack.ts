        ...
        phases: {
          ...
          build: {
            commands: [
              ...
              '  mkdir -p "build/lambda/$fn_name"',
              '  cp "$dir"/*.py "build/lambda/$fn_name/" 2>/dev/null || true',
              '  if [ -d "$dir/package" ]; then',
              '    cp -r "$dir/package/"* "build/lambda/$fn_name/" 2>/dev/null || true',
              '  fi',
              '  cd "build/lambda/$fn_name" && zip -r "../../$fn_name.zip" . && cd ../../..',
              'done',
              // CDK Synth
          ...
          post_build: {
            commands: [
              ...
              '  if [ -f "$zip" ]; then',
              '    fn_name=$(basename "$zip" .zip)',
              '    aws s3 cp "$zip" "s3://${ARTIFACT_BUCKET}/builds/${fn_name}/${TIMESTAMP}-${COMMIT_SHA}.zip"',
              '  fi',
              'done',
            ],
          },
